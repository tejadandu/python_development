def binarySearch(needle, haystack, left=None, right=None):
    ...
    if left is None:
        left = 0  # `left` is default to the 0 index.
    if right is None:
        right = len(haystack) - 1  # `right` is default to the last index.

    print("Searching: ", haystack[left : right + 1])

    if left > right:  # BASE CASE
        return None  # `needle` is not in `haystack`.
    mid = (left + right) // 2
    ...
    elif needle > haystack[mid]:  # RECUSIVE CASE
