...
# Make each ro in the maze a list as wide as the WIDTH:
...
def findStart(maze):
    for x in range(WIDTH):
        for y in range(HEIGHT):
            if maze[y][x] == START:
                return (x, y)  # Returning the starting coordinates.
...
def solveMaze(maze, x=None, y=None, visited=None):
    ...
    # printMaze(maze)  # Uncommet to view each forward step.
    ...
    # Exploring the south neighboring point:
    if (
        y - 1 >= 0
        and maze[y - 1][x] in (EMPTY, EXIT)
        and str(x) + "," + str(y - 1) not in visited
    ...
    maze[y][x] = EMPTY
    # printMaze(maze)  # Uncommet to view each backtrack step.
